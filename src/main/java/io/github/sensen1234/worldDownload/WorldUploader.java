...
public class WorldUploader {
    public static void uploadWorld(Player player, String worldName, File worldFolder) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                player.sendMessage(plugin.getMessage("compressing")); // 提示开始压缩
    ...
    private static void addSpecificFilesToZip(File folder, String parentFolder, java.util.zip.ZipOutputStream zos, Player player) throws IOException {
        ...
        if (levelDat.exists()) {
            ...
            try (FileInputStream fis = new FileInputStream(levelDat)) {
                ...
                zos.closeEntry();
                player.sendMessage("§7已添加: level.dat"); // 保留 level.dat 的提示
            }
        }
        ...
        for (File file : files) {
            if (file.isFile()) {
                ...
                if (fileName.endsWith(".mca") || fileName.endsWith(".mcr")) {
                    ...
                    try (FileInputStream fis = new FileInputStream(file)) { // 可考虑加 BufferedInputStream
                        java.util.zip.ZipEntry zipEntry = new java.util.zip.ZipEntry(zipEntryName);
                        zos.putNextEntry(zipEntry);
                        byte[] buffer = new byte[8192]; // 增大缓冲区
                        int length;
    ...
    private static String uploadFileToLegacyFawe(File file, String uuid, Player player) {
        try {
            ...
            try (DataOutputStream dos = new DataOutputStream(connection.getOutputStream())) { // 可考虑 BufferedOutputStream
                // 写入multipart头部
                ...
                // 读取文件并写入（带限速）
                try (FileInputStream fis = new FileInputStream(file)) { // 可考虑 BufferedInputStream
            ...
            if (responseCode == 302) {
                ...
                // 可以选择性地将错误响应的一部分发给玩家
                // player.sendMessage("§c上传错误响应: " + errorResponse.toString().substring(0, Math.min(100, errorResponse.length())) + "...");
            }
